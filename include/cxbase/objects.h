...
namespace cxbase {
    ...
    class CBox {
    public:
        CBox(coord_type x0, coord_type y0, coord_type x1, coord_type y1) : xl(x0), yl(y0), xh(x1), yh(y1) {}
    ...
    public:
        CRect(coord_type x0, coord_type y0, coord_type x1, coord_type y1, lay_type lay,
              purp_type purp) : layer(lay), purpose(purp), CBox(x0, y0, x1, y1) {}
        lay_type layer;
        purp_type purpose;
    };
    ...
    public:
        CPin(coord_type x0, coord_type y0, coord_type x1, coord_type y1, lay_type lay, purp_type purp, std::string net,
             std::string lbl) : CRect(x0, y0, x1, y1, lay, purp), net(std::move(net)), label(std::move(lbl)) {};
        std::string net;
        std::string label;
    };
}
