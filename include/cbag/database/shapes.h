...
struct Arc : ShapeBase {
    ...
    Arc(lay_t lay, purp_t purp, std::string net, double start, double stop)
        : ShapeBase(lay, purp, std::move(net)), ang_start(start),
          ang_stop(stop), bbox() {}

    Arc(lay_t lay, purp_t purp, std::string net, double start, double stop,
        coord_t xl, coord_t yl, coord_t xh, coord_t yh)
        : ShapeBase(lay, purp, std::move(net)), ang_start(start),
          ang_stop(stop), bbox(xl, yl, xh, yh) {}
...
struct Donut : ShapeBase {
    ...
    Donut(lay_t lay, purp_t purp, std::string net, dist_t r, dist_t hole_r,
          coord_t x, coord_t y)
        : ShapeBase(lay, purp, std::move(net)), center(x, y), radius(r),
          hole_radius(hole_r) {}
...
struct Path : ShapeBase {
    Path()
        : ShapeBase(), width(0), points(), style(psTruncate), begin_ext(0),
          end_ext(0) {}

    Path(lay_t lay, purp_t purp, std::string net, dist_t width, uint32_t n,
         PathStyle style, dist_t begin_ext, dist_t end_ext)
        : ShapeBase(lay, purp, std::move(net)), width(width), points(n),
          style(style), begin_ext(begin_ext), end_ext(end_ext) {}
...
struct TextBase : ShapeBase {
    TextBase()
        : ShapeBase(), origin(0, 0), alignment(taCC), orient(oR0), font(fRoman),
          height(0), overbar(false), visible(true), drafting(true) {}

    TextBase(lay_t lay, purp_t purp, std::string net, TextAlign align,
             Orientation orient, Font font, dist_t height, bool overbar,
             bool visible, bool drafting)
        : ShapeBase(lay, purp, std::move(net)), origin(), alignment(align),
          orient(orient), font(font), height(height), overbar(overbar),
          visible(visible), drafting(drafting) {}
...
struct Text : TextBase {
    ...
    Text(lay_t lay, purp_t purp, std::string net, std::string text,
         TextAlign align, Orientation orient, Font font, dist_t height,
         bool overbar, bool visible, bool drafting)
        : TextBase(lay, purp, std::move(net), align, orient, font, height,
...
struct EvalText : Text {
    ...
    EvalText(lay_t lay, purp_t purp, std::string net, std::string text,
             TextAlign align, Orientation orient, Font font, dist_t height,
             bool overbar, bool visible, bool drafting, std::string eval)
        : Text(lay, purp, std::move(net), std::move(text), align, orient, font,
               height, overbar, visible, drafting),
...
struct TermAttr : TextBase {
    ...
    TermAttr(TermAttrType attr_type, lay_t lay, purp_t purp, std::string net,
             TextAlign align, Orientation orient, Font font, dist_t height,
             TextDispFormat format, bool overbar, bool visible, bool drafting)
        : TextBase(lay, purp, std::move(net), align, orient, font, height,
                   overbar, visible, drafting),
...
using Shape =
    boost::variant<Rect, Poly, Arc, Donut, Ellipse, Line, Path, Text, EvalText>;
} // namespace cbag
