...
class make_pin_fig_visitor : public boost::static_visitor<> {
  public:
    make_pin_fig_visitor(const oa::oaCdbaNS *ns, oa::oaBlock *block,
                         oa::oaPin *pin, oa::oaTerm *term, int *cnt)
        : ns(ns), block(block), pin(pin), term(term), cnt(cnt) {}

    void operator()(const cbag::Rect &operand) const {
        oa::oaRect *rect = oa::oaRect::create(block, operand.layer,
                                              operand.purpose, operand.bbox);
        rect->addToPin(pin);
    }
    ...
    void operator()(const cbag::SchPinObject &obj) const {
        ...
        oa::oaScalarInst *inst = oa::oaScalarInst::create(
            block, lib, cell, view, name, obj.inst.xform);
        inst->addToPin(pin);

        oa::oaAttrDisplay::create(
            term, obj.attr.attr_type, obj.attr.layer, obj.attr.purpose,
            obj.attr.origin, obj.attr.alignment, obj.attr.orient, obj.attr.font,
            obj.attr.height, obj.attr.format, obj.attr.overbar,
...
class make_shape_visitor : public boost::static_visitor<> {
  public:
    ...
    void operator()(const cbag::Arc &v) const {
        oa::oaShape *p = oa::oaArc::create(block, v.layer, v.purpose, v.bbox,
                                           v.ang_start, v.ang_stop);
        add_shape_to_net(p, v.net);
    }

    void operator()(const cbag::Donut &v) const {
        oa::oaShape *p = oa::oaDonut::create(block, v.layer, v.purpose,
                                             v.center, v.radius, v.hole_radius);
        add_shape_to_net(p, v.net);
    }
    ...
    void operator()(const cbag::Path &v) const {
        oa::oaShape *p =
            oa::oaPath::create(block, v.layer, v.purpose, v.width, v.points,
                               v.style, v.begin_ext, v.end_ext);
        add_shape_to_net(p, v.net);
    }

    void operator()(const cbag::Text &v) const {
        oa::oaShape *p = oa::oaText::create(
            block, v.layer, v.purpose, v.text.c_str(), v.origin, v.alignment,
    ...
    void operator()(const cbag::EvalText &v) const {
        oa::oaShape *p = oa::oaEvalText::create(
            block, v.layer, v.purpose, v.text.c_str(), v.origin, v.alignment,
            v.orient, v.font, v.height, v.evaluator.c_str(), v.overbar,
...
void OAWriter::create_terminal_pin(
    oa::oaBlock *block, int &pin_cnt,
    ...
    for (auto const &pair : map) {
        ...
        if (term_net == nullptr || term_net->isImplicit()) {
            term_net =
                oa::oaNet::create(block, term_name, pair.second.sig_type);
        }
        ...
        boost::apply_visitor(
            make_pin_fig_visitor(&ns, block, pin, term, &pin_cnt),
            pair.second.obj);
    }
}

void OAWriter::write_sch_cellview(const cbag::SchCellView &cv,
                                  oa::oaDesign *dsn) {
    ...
    for (auto const &pair : cv.instances) {
        cbag::spirit::ast::name_unit nu =
            cbag::parse_cdba_name_unit(pair.first);
        ...
        oa::oaInst *ptr;
        if (nu.is_vector()) {
            ptr = oa::oaVectorInst::create(
                block, lib, cell, view, name, nu.idx_range.start,
                nu.idx_range.stop, pair.second.xform);
        } else {
            ptr = oa::oaScalarInst::create(block, lib, cell, view, name,
                                           pair.second.xform);
        }
        ...
        for (auto const &term_net_pair : pair.second.connections) {
            LOG(INFO) << "Connecting inst " << pair.first << " terminal "
                      << term_net_pair.first << " to " << term_net_pair.second;
        ...
        for (auto const &prop_pair : pair.second.params) {
            boost::apply_visitor(make_prop_visitor(ptr, prop_pair.first),
                                 prop_pair.second);
        }
    }
    ...
    for (auto const &prop_pair : cv.props) {
        ...
        if (prop_pair.first != "lastSchematicExtraction") {
            boost::apply_visitor(make_prop_visitor(dsn, prop_pair.first),
                                 prop_pair.second);
        }
    }
    ...
    for (auto const &prop_pair : cv.app_defs) {
        boost::apply_visitor(make_app_def_visitor(dsn, prop_pair.first),
                             prop_pair.second);
    }
    ...
    */
    oa::oaTimeProp::create(dsn, "lastSchematicExtraction",
                           dsn->getLastSavedTime());
