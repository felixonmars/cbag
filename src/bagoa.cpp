...
namespace cxbase {
    ...
    void OALayoutLibrary::create_layout(const std::string &cell, const std::string &view,
        ...
        try {
            ...
            for (bag::PathSegIter it = layout.path_seg_list.begin();
                 it != layout.path_seg_list.end();
                 it++) {
            ...
            for (bag::PolygonIter it = layout.polygon_list.begin();
                 it != layout.polygon_list.end(); it++) {
                create_polygon(blk_ptr, *it);
            }
            for (bag::BlockageIter it = layout.block_list.begin();
                 it != layout.block_list.end(); it++) {
                create_blockage(blk_ptr, *it);
            }
            for (bag::BoundaryIter it = layout.boundary_list.begin();
                 it != layout.boundary_list.end(); it++) {
    ...
    void OALayoutLibrary::create_inst(oa::oaBlock *blk_ptr, const bag::Inst &inst) {
        ...
        for (bag::DoubleIter it = inst.double_params.begin();
             it != inst.double_params.end(); it++) {
            oa::oaString key(it->first.c_str());
            oa_params.append(oa::oaParam(key, it->second));
        }
    ...
    void OALayoutLibrary::create_via(oa::oaBlock *blk_ptr, const bag::Via &inst) {
        ...
        oa::oaStdViaDef *vdef = static_cast<oa::oaStdViaDef *>(oa::oaViaDef::find(tech_ptr,
                                                                                  oa_via_id));
        if (vdef == NULL) {
        ...
        oa::oaFig *fig = static_cast<oa::oaFig *>(oa::oaStdVia::create(blk_ptr, vdef, xfm,
                                                                       &params));
    ...
    void OALayoutLibrary::create_rect(oa::oaBlock *blk_ptr, const bag::Rect &inst) {
        ...
        if (purp_iter == purp_map.end()) {
            std::cout << "create_rect: unknown purpose " << inst.purpose << ", skipping."
                      << std::endl;
            return;
        }
    ...
    void OALayoutLibrary::create_path_seg(oa::oaBlock *blk_ptr, const bag::PathSeg &inst) {
        LayerIter lay_iter = lay_map.find(inst.layer);
        if (lay_iter == lay_map.end()) {
            std::cout << "create_path_seg: unknown layer " << inst.layer << ", skipping."
                      << std::endl;
            return;
        }
        oa::oaLayerNum layer = lay_iter->second;
    ...
    void OALayoutLibrary::create_pin(oa::oaBlock *blk_ptr, const bag::Pin &inst) {
        ...
        PurposeIter purp_iter = purp_map.find(inst.purpose);
        if (purp_iter == purp_map.end()) {
            std::cout << "create_pin: unknown purpose " << inst.purpose << ", skipping."
                      << std::endl;
            return;
        }
        oa::oaPurposeNum purpose = purp_iter->second;
        ...
        oa::oaText::create(blk_ptr, layer, purpose, oa_label, op, oa::oacCenterCenterTextAlign,
                           lorient,
    ...
    void OALayoutLibrary::create_polygon(oa::oaBlock *blk_ptr, const bag::Polygon &inst) {
        ...
        LayerIter lay_iter = lay_map.find(inst.layer);
        if (lay_iter == lay_map.end()) {
            std::cout << "create_polygon: unknown layer " << inst.layer << ", skipping."
                      << std::endl;
            return;
        }
        oa::oaLayerNum layer = lay_iter->second;
        ...
        PurposeIter purp_iter = purp_map.find(inst.purpose);
        if (purp_iter == purp_map.end()) {
            std::cout << "create_polygon: unknown purpose " << inst.purpose << ", skipping."
                      << std::endl;
            return;
        }
        oa::oaPurposeNum purpose = purp_iter->second;
    ...
    void OALayoutLibrary::create_blockage(oa::oaBlock *blk_ptr, const bag::Blockage &inst) {
        ...
        if (inst.type == "placement") {
            ...
            LayerIter lay_iter = lay_map.find(inst.layer);
            if (lay_iter == lay_map.end()) {
                std::cout << "create_blockage: unknown layer " << inst.layer << ", skipping."
                          << std::endl;
                return;
            }
            oa::oaLayerNum layer = lay_iter->second;
    ...
    void OALayoutLibrary::create_boundary(oa::oaBlock *blk_ptr, const bag::Boundary &inst) {
        ...
        if (inst.type == "PR") {
            ...
            std::cout << "create_boundary: unknown boundary type" << inst.type << ", skipping."
                      << std::endl;
        }
    }
        ...
        try {
            ...
            for (std::vector<bag::SchCell>::const_iterator it = cell_list.begin();
                ...
                oa::oaDesign *dsn_ptr = oa::oaDesign::open(sch_lib, cell_name, sch_view,
                                                           oa::oaViewType::get(oa::oacSchematic),
                                                           'r');
                ...
                oa::oaStringProp *prop_ptr = static_cast<oa::oaStringProp *>(oa::oaProp::find(
                        dsn_ptr,
                        part_name_prop));
                if (prop_ptr != NULL) {
